...
export async function authenticateProviderAccess(phoneOrWhatsapp: string, token: string): Promise<Provider | null> {
  ...
  return (
    providers.find(
      (provider) =>
        ...
        provider.verification.managementToken === normalizedToken &&
